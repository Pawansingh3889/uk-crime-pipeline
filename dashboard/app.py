...
# ── Row 1: Monthly trend + Crime by city ────────────────────────────────────
...
with col1:
    ...
    fig = px.line(
        ...
        x="month", y="total_crimes", color="city",
        markers=True,
...
# ── Row 2: Crime category breakdown ─────────────────────────────────────────
...
with col4:
    city_cats = (
        by_city[by_city["city"] == selected_city]
        .groupby("category")["crime_count"]
        .sum()
        .reset_index()
...
city_hotspots = (
    hotspots[hotspots["city"] == hotspot_city]
...
st.caption("Data source: Police UK API (data.police.uk) · Pipeline: Python → PostgreSQL → dbt → Streamlit")